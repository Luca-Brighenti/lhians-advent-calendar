...
If Day 11 says "Uploads are not configured yet":
- Make sure `config.js` exists at the site root (same folder as index.html).
- Edit `config.js` and set:
      window.ADVENT_CONFIG = { upload: { getformEndpoint: "https://getform.io/f/YOUR-ENDPOINT" } };
- Confirm your HTML loads it *before* `js/script.js` (this version does).
- Push the changes and hard-refresh the page (Ctrl+F5). GitHub Pages may cache files for a minute.
